...
repo = 'https://tecnoshare.sharepoint.com/sites/beer/';

function updateLebels(beers, repo) {
  for (let i in beers) {
    lab = beers[i].label.split("/");
    lab.pop();
    folder = lab.pop();
    fname = beers[i].fname;
    beers[i].label = repo + folder  + '/' + fname + '.png'
  }
}

updateLebels(beers, repo);